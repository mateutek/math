...
import { BUILDING_ART, PROJ, fitArt, spriteFile } from '@/data/villageMap'
...
// the goal's own sprites, cropped into the card's 72 x 69 art box exactly as
// the design board crops them
const art = computed(() =>
  goal.value ? fitArt(BUILDING_ART[goal.value.id][goal.value.tier - 1], 72, 69) : [],
)
const url = (p) => `${import.meta.env.BASE_URL}village/${spriteFile(p.name, p.facing)}`
